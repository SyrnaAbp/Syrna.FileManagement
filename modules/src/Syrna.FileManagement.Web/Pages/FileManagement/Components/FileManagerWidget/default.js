(function ($) {
    ...
    abp.widgets.FileManager = function ($widget) {
        ...
        function initFileManagerTable(filters) {
            ...
            if (canDeleteFile === null) canDeleteFile = abp.auth.isGranted('Syrna.FileManagement.File.Delete');
            if (canDeleteDirectory === null) canDeleteDirectory = abp.auth.isGranted('Syrna.FileManagement.File.Delete');
            if (canMoveFile === null) canMoveFile = abp.auth.isGranted('Syrna.FileManagement.File.Move');
            if (canMoveDirectory === null) canMoveDirectory = abp.auth.isGranted('Syrna.FileManagement.File.Move');
            if (canRenameFile === null) canRenameFile = abp.auth.isGranted('Syrna.FileManagement.File.Update');
            if (canRenameDirectory === null) canRenameDirectory = abp.auth.isGranted('Syrna.FileManagement.File.Update');
            if (canDownloadFile === null) canDownloadFile = abp.auth.isGranted('Syrna.FileManagement.File.GetDownloadInfo');

            var l = abp.localization.getResource('SyrnaFileManagement');

            var service = syrna.fileManagement.files.file;
                                ...
                                [
                                    {
                                        ...
                                        action: function (data) {
                                            syrna.fileManagement.files.file.getDownloadInfo(data.record.id, {
